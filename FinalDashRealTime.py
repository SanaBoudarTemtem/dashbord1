...
BUCKET_NAME = "one-data-lake"  # Replace with your S3 bucket name
FILE_KEY = "csv_database/submissions_part_1.csv.gz"  # Replace with the S3 file path (e.g., "data/file.csv")

# Function to load file from S3
def load_file_from_s3(bucket_name, file_key, aws_access_key_id, aws_secret_access_key):
    s3 = boto3.client(
        's3',
    ...
    try:
        ...
        # Check if file is CSV or Excel and load into pandas DataFrame
        ...
        st.error(f"Error loading file from S3: {str(e)}")
        return None
...
# Load the dataset from S3 instead of using file uploader
df = load_file_from_s3(BUCKET_NAME, FILE_KEY, AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY)

if df is not None:
    st.sidebar.success(f"Successfully loaded file from S3: {FILE_KEY}")
    ...
    color_discrete_sequence=color_scheme
        ...
        with col1:
            st.markdown(f"<div class='kpi-card'><div class='kpi-title'>Min Cashback per ticket💵</div><div class='kpi-value'>{min_cashback_per_ticket:,.2f} DZD</div></div>", unsafe_allow_html=True)

        with col2:
            st.markdown(f"<div class='kpi-card'><div class='kpi-title'>Max Cashback per ticket💸</div><div class='kpi-value'>{max_cashback_per_ticket:,.2f} DZD</div></div>", unsafe_allow_html=True)
...
# Ensure the 'createdAt_challengesubmissions' is in datetime format
    ...
    color_discrete_sequence=color_scheme     # Custom color scheme
        ...
        # Vérifier si la colonne "name" (nom de l'entreprise) existe dans le fichier
        if 'name' in df.columns and 'title.fr' in df.columns:
            # Nombre total d'entreprises
            total_enterprises = df['name'].nunique()

            # L'entreprise avec le plus de challenges (produits)
            top_enterprise_by_challenges = df['name'].value_counts().idxmax()

            # Calculer les soumissions par entreprise
            submissions_by_enterprise = df.groupby('name')['submittedBy.id'].count().reset_index()
            submissions_by_enterprise.columns = ['Entreprise', 'Total Submissions']

            # L'entreprise avec le plus de soumissions
            top_enterprise_by_submissions = submissions_by_enterprise.loc[submissions_by_enterprise['Total Submissions'].idxmax(), 'Entreprise']

            # Calculer le cashback total par entreprise
            if 'Cashback Crédité' in df.columns:
                cashback_by_enterprise = df.groupby('name')['Cashback Crédité'].sum().reset_index()
                cashback_by_enterprise.columns = ['Entreprise', 'Total Cashback']
            else:
                cashback_by_enterprise = pd.DataFrame(columns=['Entreprise', 'Total Cashback'])

            # Afficher les KPI cards
            col1, col2, col3= st.columns(3)
            
            with col1:
                st.markdown(f"<div class='kpi-card'><div class='kpi-title'>Total Enterprises 🏢</div><div class='kpi-value'>{total_enterprises}</div></div>", unsafe_allow_html=True)
            
            with col2:
                st.markdown(f"<div class='kpi-card'><div class='kpi-title'>Top Enterprise by Challenges 🚀</div><div class='kpi-value'>{top_enterprise_by_challenges}</div></div>", unsafe_allow_html=True)
            
            with col3:
                st.markdown(f"<div class='kpi-card'><div class='kpi-title'>Top Enterprise by Submissions 📊</div><div class='kpi-value'>{top_enterprise_by_submissions}</div></div>", unsafe_allow_html=True)
            
            # with col4:
            #     total_cashback_distributed = cashback_by_enterprise['Total Cashback'].sum()
            #     st.markdown(f"<div class='kpi-card'><div class='kpi-title'>Total Cashback Distributed 💰</div><div class='kpi-value'>{total_cashback_distributed:,.2f} DZD</div></div>", unsafe_allow_html=True)

            # Ajouter un tableau des entreprises avec le budget consommé (total cashback)
            st.subheader("Enterprise Budget Consumed (Total Cashback)")
            enterprise_budget_consumed = df.groupby('name')['Cashback Crédité'].sum().reset_index()
            enterprise_budget_consumed.columns = ['Enterprise', 'Total Cashback']
            st.dataframe(enterprise_budget_consumed, use_container_width=True)

            # Ajouter un sélecteur pour choisir une entreprise
            selected_enterprise = st.selectbox("Select an Enterprise", df['name'].unique())

            # Filtrer les données pour l'entreprise sélectionnée
            enterprise_data = df[df['name'] == selected_enterprise]

            # Calculer les statistiques par produit (challenge) pour cette entreprise
            product_stats = enterprise_data.groupby('title.fr').agg(
                total_submissions=('submittedBy.id', 'size'),
                total_cashback=('Cashback Crédité', 'sum')
            ).reset_index()
            product_stats.columns = ['Product (Challenge)', 'Total Submissions', 'Total Cashback']

            # Afficher un tableau avec les informations sur les produits de l'entreprise
            st.subheader(f"Products (Challenges) of {selected_enterprise}")
            st.dataframe(product_stats, use_container_width=True)

            # Ajouter un graphique en histogramme montrant le nombre de soumissions par entreprise
            st.subheader("Submissions by Enterprise")
            fig_submissions_histogram = px.histogram(
                submissions_by_enterprise,
                x='Entreprise',
                y='Total Submissions',
                title="Number of Submissions by Enterprise",
                labels={'Entreprise': 'Enterprise', 'Total Submissions': 'Submissions'},
                color_discrete_sequence=color_scheme
            )
            st.plotly_chart(fig_submissions_histogram, use_container_width=True)



            # Visualisation : Cashback total par entreprise
            fig_cashback_by_enterprise = px.bar(
                cashback_by_enterprise,
                x='Entreprise',
                y='Total Cashback',
                title="Total Cashback Distributed by Enterprise",
                labels={'Entreprise': 'Enterprise', 'Total Cashback': 'Cashback (DZD)'},
                color_discrete_sequence=color_scheme
            )
            st.plotly_chart(fig_cashback_by_enterprise, use_container_width=True)

            # Visualisation : Cashback par entreprise et par challenge (produit)
            if 'Cashback Crédité' in df.columns and 'title.fr' in df.columns:
                cashback_by_enterprise_and_product = df.groupby(['name', 'title.fr'])['Cashback Crédité'].sum().reset_index()
                cashback_by_enterprise_and_product.columns = ['Entreprise', 'Challenge', 'Total Cashback']

                fig_cashback_by_product = px.bar(
                    cashback_by_enterprise_and_product,
                    x='Entreprise',
                    y='Total Cashback',
                    color='Challenge',
                    title="Total Cashback by Enterprise and Product",
                    labels={'Entreprise': 'Enterprise', 'Total Cashback': 'Cashback (DZD)', 'Challenge': 'Challenge (Product)'},
                    barmode='stack',
                    color_discrete_sequence=color_scheme
                )
                st.plotly_chart(fig_cashback_by_product, use_container_width=True)
        
        else:
            st.warning("The necessary columns ('name', 'title.fr', 'Cashback Crédité') are missing in the dataset.")
...
# Display Top 10 Products by Submissions in the first column
    ...
    # Display KPIs for Users
        ...
        # Trier le tableau par nombre d'utilisateurs uniques, en ordre décroissant
            ...
            user_distribution = user_distribution.sort_values(by='Unique Users', ascending=False).set_index('Wilaya')
        ...
        # Trier le tableau par nombre d'utilisateurs, en ordre décroissant
            users_per_challenge = users_per_challenge.sort_values(by='Unique Users', ascending=False).set_index('Challenge')
