...
# Ensure the 'createdAt_challengesubmissions' is in datetime format
    ...
    color_discrete_sequence=color_scheme     # Custom color scheme
        ...
        if 'name' in df.columns and 'title.fr' in df.columns:
            ...
            # Afficher le tableau des soumissions par entreprise
            st.subheader("Submissions by Enterprise")
            st.dataframe(submissions_by_enterprise, use_container_width=True)

            # Afficher le tableau du cashback par entreprise
            st.subheader("Cashback by Enterprise")
            st.dataframe(cashback_by_enterprise, use_container_width=True)

            # Visualisation : Nombre de soumissions par entreprise
            fig_submissions_by_enterprise = px.bar(
                submissions_by_enterprise,
                x='Entreprise',
                y='Total Submissions',
                title="Total Submissions by Enterprise",
                labels={'Entreprise': 'Enterprise', 'Total Submissions': 'Submissions'},
                color_discrete_sequence=color_scheme
            )
            st.plotly_chart(fig_submissions_by_enterprise, use_container_width=True)
